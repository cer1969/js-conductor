    ...
    */
    ...
    getTc(ta: number, ic: number): number {
        ...
        */
        check(ta).ge(k.TA_MIN).le(k.TA_MAX);
        var _Imin = 0;
        var _Imax = this.getCurrent(ta, k.TC_MAX);
        ...
        while ((Tmax - Tmin) > this._deltaTemp) {
            ...
            if (cuenta > k.ITER_MAX) {
                var err_msg = `getTc(): N° iterations > ${k.ITER_MAX}`;
    ...
    getTa(tc: number, ic: number): number {
        ...
        */
        ...
        var _Imin = this.getCurrent(k.TA_MAX, tc);
        var _Imax = this.getCurrent(k.TA_MIN, tc);
        check(ic).ge(_Imin).le(_Imax);  // Ensure TA_MIN =< Ta =< TA_MAX
        
        var Tmin = k.TA_MIN;
        var Tmax = Math.min(k.TA_MAX, tc);
        if (Tmin >= Tmax) {
            return tc;
        }
        ...
        while ((Tmax - Tmin) > this._deltaTemp) {
            ...
            cuenta = cuenta + 1;
            if (cuenta > k.ITER_MAX) {
                var err_msg = `getTa(): N° iterations > ${k.ITER_MAX}`;
                throw new RangeError(err_msg);
            }
        }
        return Tmed;
